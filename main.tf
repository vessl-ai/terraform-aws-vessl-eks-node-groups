...
module "worker_node_groups" {
  ...
  bootstrap_extra_args = join(" ", [
    "--kubelet-extra-args",
    "'",
    length(each.value["node_template_labels"]) == 0 ? "--node-labels=app.vessl.ai/managed=true" : "--node-labels=app.vessl.ai/managed=true,${join(",", [for k, v in each.value["node_template_labels"] : "${k}=${v}"])}",
    length(each.value["taints"]) == 0 ? "" : "--register-with-taints=${join(",", each.value["taints"])}",
    "'",
  ])
  ...
  iam_instance_profile_arn    = aws_iam_instance_profile.node_group.arn
  ...
  autoscaling_group_tags = merge(
    {
      for k, v in each.value["node_template_labels"]: "k8s.io/cluster-autoscaler/node-template/label/${k}" => v
    },
    {
      for k, v in each.value["node_template_resources"]: "k8s.io/cluster-autoscaler/node-template/resources/${k}" => v
    },
    {
...
module "manager_node_group" {
  ...
  bootstrap_extra_args = join(" ", [
    "--kubelet-extra-args",
    "'",
    "--node-labels=app.vessl.ai/dedicated=manager,",
